...
Also: `proxy.ts` middleware calls `supabase.auth.getUser()` on every route; offline it retries (`AuthRetryableFetchError`) and can stall SSR. It now has a 2.5s timeout + fail-closed fallback. Related: [[koraspace-landing-rebuild]] if that memory exists.
